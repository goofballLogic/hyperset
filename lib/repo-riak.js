...
(function(context) {
	...
	context.Repo = function(hostname, port, setPrefix) {
		...
		var I = {
			...
			"storeItemMeta" : storeItemMeta,
			"retrieve" : retrieveItem,
			"retrieveSetMeta" : retrieveSetMeta,
			"retrieveItemMeta" : retrieveItemMeta,
			"index" : index,
			"remove" : remove,
		...
		function storeItemMeta(setName, itemId, meta, callback) {
			store(setName, "meta.item-" + itemId, meta, {}, callback);
		}
		...
		function retrieveItemMeta(setName, itemId, callback) {
			retrieve(setName, "meta.item-" + itemId, callback);
		}
		...
		function remove(setName, itemId, callback) {
			agent.del(setName, "item-" + itemId, callback);
		}
