...
var aee = require("./asyncEventEmitter");

(function(context) {

	context.Riak = require("./repo-riak");
	context.Express = require("./adapter-express");
	...
	context.Sets = function(config, options) {
		...
		var ret = this;
		var events = new aee.AsyncEventEmitter();
		...
		var sets = {
			emit: function() { events.emit.apply(ret, arguments); }
		};

		function buildModuleEventProxy(test, handler) {
			return function() {
				if(!test || (test.apply(this, arguments))) handler.apply(this, arguments);
				else { var callback = arguments[arguments.length-1]; if(typeof callback == "function") callback(); }
			};
		}

		if(config.modules) {
			forEach(config.modules, function(moduleDef) {
				var module = require(moduleDef.name);
				forEach(Object.keys(module), function(eventHandler) {
					if(eventHandler.length > 2 && eventHandler.substring(0, 2) == "on") {
						var eventName = eventHandler.substring(2).toLowerCase();
						forEach(module[eventHandler], function(def) {
							var proxy = buildModuleEventProxy(def.test, def.handler);
							events.on(eventName, proxy);
						});
					}
				});
			});
	...
	function Link(rel, followAction, resource, path) {
		...
		this.events = new aee.AsyncEventEmitter();
		this.action = function() {
			return followAction.apply(resource, arguments);
		};
		this.follow = function() {

			var followArgs = Array.prototype.slice.call(arguments, 0), callContext = this;

			// create (or wrap) callback function?
			var originalCallback = function() { };
			if(typeof followArgs[followArgs.length - 1] == "function") originalCallback = followArgs.pop();

			// append the callback
			followArgs.push(function() {
				var callbackArgs = arguments;
				var completion = function() { originalCallback.apply(callContext, callbackArgs); };
				self.events.emit("followed", self, resource, callbackArgs, function() {
					if(resource && resource.emit)
						resource.emit("followed", self, resource, callbackArgs, completion);
					else completion();
				});
			});

			// emit the link's following event
			self.events.emit("following", self, resource, followArgs, function() {
				var completion = function() { self.action.apply(callContext, followArgs); };
				// emit the high-level following event, and run action to completion
				if((resource!==context.NRR) && resource && resource.emit)
					resource.emit("following", self, resource, followArgs, completion);
				else completion();
			});

		};
		this.clone = function() {
			return new Link(this.rel, this.follow, resource, path);
		};
	...
	function Hypermedia() {
		...
		this.emit = function() {
			if(baseSets.emit) baseSets.emit.apply(this, arguments);
			else {
