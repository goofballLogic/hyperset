...
(function(context) {
	...
	context.Sets = function(config, options) {
		...
		var sets = {
			emit: function() { ret.emit.apply(ret, arguments); }
		};
	...
	context.Sets.prototype = new events.EventEmitter();
		...
		this.follow = function() {
			...
			if(typeof args[args.length - 1] == "function") {
				...
				args[args.length - 1] = function() {
					self.events.emit("followed", self, resource, arguments);
					if(resource && resource.emit) resource.emit("followed", self, resource, arguments);
					return originalCallback.apply(this, arguments);
				};
			}
			self.events.emit("following", self, resource, arguments);
			if(resource && resource.emit) resource.emit("following", self, resource, arguments);
			return self.action.apply(this, args);
		};
		this.clone = function() {
	...
	function Hypermedia() {
		...
		this.emit = function() { if(baseSets.emit) baseSets.emit.apply(this, arguments); };
